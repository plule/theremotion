...
impl FaustDsp for mydsp {
	...
	fn get_param(&self, param: ParamIndex) -> Option<Self::T> {
		match param.0 {
			...
			14 => Some(self.fHslider17),
			15 => Some(self.fHslider18),
			17 => Some(self.fHslider19),
			0 => Some(self.fHslider2),
			3 => Some(self.fHslider3),
			20 => Some(self.fHslider4),
	...
	fn set_param(&mut self, param: ParamIndex, value: Self::T) {
		match param.0 {
			...
			14 => { self.fHslider17 = value }
			15 => { self.fHslider18 = value }
			17 => { self.fHslider19 = value }
			0 => { self.fHslider2 = value }
			3 => { self.fHslider3 = value }
			20 => { self.fHslider4 = value }
	...
	fn compute(&mut self, count: i32, inputs: &[&[Self::T]], outputs: &mut[&mut[Self::T]]) {
		...
		let mut fSlow16: F32 = 2.0 * ((self.fHslider16) as F32);
		let mut fSlow17: F32 = self.fConst2 * ((self.fHslider17) as F32);
		let mut fSlow18: F32 = self.fConst2 * ((self.fHslider18) as F32);
		let mut fSlow19: F32 = ((self.fButton0) as F32);
		let mut fSlow20: F32 = 0.00100000005 * ((self.fHslider19) as F32);
		let mut iSlow21: i32 = ((F32::abs(fSlow20) < 1.1920929e-07) as i32);
		let mut fThen25: F32 = F32::exp(0.0 - self.fConst5 / if (iSlow21 as i32 != 0) { 1.0 } else { fSlow20 });
		let mut fSlow22: F32 = 0.0 - if (iSlow21 as i32 != 0) { 0.0 } else { fThen25 };
		...
		for (output0, output1) in zipped_iterators {
			...
			let mut fTemp12: F32 = F32::max(0.0, F32::min(2047.0, self.fConst6 / fTemp11));
			let mut fTemp13: F32 = F32::floor(fTemp12);
			let mut fTemp14: F32 = F32::max(20.0, F32::abs(fTemp11));
			let mut fTemp15: F32 = self.fRec6[1] + self.fConst5 * fTemp14;
			self.fRec6[0] = fTemp15 - F32::floor(fTemp15);
			let mut fTemp16: F32 = mydsp_faustpower2_f(2.0 * self.fRec6[0] + -1.0);
			self.fVec3[0] = fTemp16;
			let mut fTemp17: F32 = (fTemp0 * (fTemp16 - self.fVec3[1])) / fTemp14;
			self.fVec4[(self.IOTA0 & 4095) as usize] = fTemp17;
			let mut iTemp18: i32 = ((fTemp12) as i32);
			self.fRec5[0] = 0.999000013 * self.fRec5[1] - self.fConst4 * ((fTemp12 - fTemp13) * self.fVec4[((self.IOTA0 - (iTemp18 + 1)) & 4095) as usize] - fTemp17 - self.fVec4[((self.IOTA0 - iTemp18) & 4095) as usize] * (fTemp13 + 1.0 - fTemp12));
			...
			let mut fThen1: F32 = self.fRec15[1] + fTemp39 + fTemp41 * (1.0 - self.fConst0 / fTemp38);
			...
			let mut fTemp50: F32 = self.fConst5 * fTemp49;
			let mut fTemp51: F32 = self.fRec22[1] + fTemp50;
			let mut fTemp52: F32 = fTemp51 + -1.0;
			let mut iTemp53: i32 = ((fTemp52 < 0.0) as i32);
			self.fRec22[0] = if (iTemp53 as i32 != 0) { fTemp51 } else { fTemp52 };
			let mut fThen5: F32 = self.fRec22[1] + fTemp50 + fTemp52 * (1.0 - self.fConst0 / fTemp49);
			let mut fRec23: F32 = if (iTemp53 as i32 != 0) { fTemp51 } else { fThen5 };
			self.fRec24[0] = fSlow8 + self.fConst3 * self.fRec24[1];
			let mut fTemp54: F32 = F32::min(1.41419947, 1.41421354 * self.fRec24[0]);
			let mut fTemp55: F32 = fTemp54 * (fTemp54 + 1.41421354);
			self.fRec25[0] = fSlow9 + self.fConst3 * self.fRec25[1];
			let mut fTemp56: F32 = F32::tan(self.fConst8 * F32::max(20.0, F32::min(10000.0, 440.0 * F32::powf(2.0, 0.0833333358 * (self.fRec17[0] + self.fRec25[0] + -69.0)))));
			let mut fTemp57: F32 = 1.0 / fTemp56;
			let mut fTemp58: F32 = 1.41421354 * fTemp54;
			let mut fTemp59: F32 = fTemp58 + 2.0;
			let mut fTemp60: F32 = 1.0 - 1.0 / mydsp_faustpower2_f(fTemp56);
			let mut fTemp61: F32 = fTemp55 + (fTemp57 + fTemp59) / fTemp56 + 1.0;
			self.fRec14[0] = 2.0 * fRec16 + self.fRec18[0] * (0.0 - 2.0 * (1.0 - (fRec20 + fRec23))) + -1.0 - (self.fRec14[2] * (fTemp55 + (fTemp57 - fTemp59) / fTemp56 + 1.0) + 2.0 * self.fRec14[1] * (fTemp55 + fTemp60)) / fTemp61;
			let mut fTemp62: F32 = fTemp54 * (fTemp54 + -1.41421354);
			let mut fTemp63: F32 = 2.0 - fTemp58;
			let mut fTemp64: F32 = fTemp62 + (fTemp63 + fTemp57) / fTemp56 + 1.0;
			self.fRec13[0] = (self.fRec14[2] + self.fRec14[0] + 2.0 * self.fRec14[1]) / fTemp61 - (self.fRec13[2] * (fTemp62 + (fTemp57 - fTemp63) / fTemp56 + 1.0) + 2.0 * self.fRec13[1] * (fTemp62 + fTemp60)) / fTemp64;
			self.fRec26[0] = fSlow10 + self.fConst3 * self.fRec26[1];
			self.fRec31[0] = fSlow11 + self.fConst3 * self.fRec31[1];
			let mut fTemp65: F32 = F32::powf(2.0, 0.0833333358 * (fSlow4 + self.fRec31[0] + -69.0));
			let mut fTemp66: F32 = F32::max(1.1920929e-07, F32::abs(440.0 * fTemp65));
			let mut fTemp67: F32 = self.fConst5 * fTemp66;
			let mut fTemp68: F32 = self.fRec29[1] + fTemp67;
			let mut fTemp69: F32 = fTemp68 + -1.0;
			let mut iTemp70: i32 = ((fTemp69 < 0.0) as i32);
			self.fRec29[0] = if (iTemp70 as i32 != 0) { fTemp68 } else { fTemp69 };
			let mut fThen7: F32 = fTemp67 + self.fRec29[1] + fTemp69 * (1.0 - self.fConst0 / fTemp66);
			let mut fRec30: F32 = if (iTemp70 as i32 != 0) { fTemp68 } else { fThen7 };
			let mut fTemp71: F32 = F32::max(1.1920929e-07, F32::abs(440.0 * fTemp43 * fTemp65));
			let mut fTemp72: F32 = self.fConst5 * fTemp71;
			let mut fTemp73: F32 = self.fRec32[1] + fTemp72;
			let mut fTemp74: F32 = fTemp73 + -1.0;
			let mut iTemp75: i32 = ((fTemp74 < 0.0) as i32);
			self.fRec32[0] = if (iTemp75 as i32 != 0) { fTemp73 } else { fTemp74 };
			let mut fThen9: F32 = fTemp72 + self.fRec32[1] + fTemp74 * (1.0 - self.fConst0 / fTemp71);
			let mut fRec33: F32 = if (iTemp75 as i32 != 0) { fTemp73 } else { fThen9 };
			let mut fTemp76: F32 = F32::max(1.1920929e-07, F32::abs(440.0 * fTemp48 * fTemp65));
			let mut fTemp77: F32 = self.fRec34[1] + self.fConst5 * fTemp76;
			...
			let mut fThen11: F32 = fTemp77 + fTemp78 * (1.0 - self.fConst0 / fTemp76);
			let mut fRec35: F32 = if (iTemp79 as i32 != 0) { fTemp77 } else { fThen11 };
			let mut fTemp80: F32 = F32::tan(self.fConst8 * F32::max(20.0, F32::min(10000.0, 440.0 * F32::powf(2.0, 0.0833333358 * (self.fRec25[0] + self.fRec31[0] + -69.0)))));
			let mut fTemp81: F32 = 1.0 / fTemp80;
			let mut fTemp82: F32 = 1.0 - 1.0 / mydsp_faustpower2_f(fTemp80);
			let mut fTemp83: F32 = fTemp55 + (fTemp59 + fTemp81) / fTemp80 + 1.0;
			self.fRec28[0] = 2.0 * fRec30 + self.fRec18[0] * (0.0 - 2.0 * (1.0 - (fRec33 + fRec35))) + -1.0 - (self.fRec28[2] * (fTemp55 + 1.0 - (fTemp59 - fTemp81) / fTemp80) + 2.0 * self.fRec28[1] * (fTemp55 + fTemp82)) / fTemp83;
			let mut fTemp84: F32 = fTemp62 + (fTemp63 + fTemp81) / fTemp80 + 1.0;
			self.fRec27[0] = (self.fRec28[2] + self.fRec28[0] + 2.0 * self.fRec28[1]) / fTemp83 - (self.fRec27[2] * (fTemp62 + (fTemp81 - fTemp63) / fTemp80 + 1.0) + 2.0 * self.fRec27[1] * (fTemp62 + fTemp82)) / fTemp84;
			self.fRec36[0] = fSlow12 + self.fConst3 * self.fRec36[1];
			self.fRec41[0] = fSlow13 + self.fConst3 * self.fRec41[1];
			let mut fTemp85: F32 = F32::powf(2.0, 0.0833333358 * (fSlow4 + self.fRec41[0] + -69.0));
			let mut fTemp86: F32 = F32::max(1.1920929e-07, F32::abs(440.0 * fTemp85));
			let mut fTemp87: F32 = self.fConst5 * fTemp86;
			let mut fTemp88: F32 = self.fRec39[1] + fTemp87;
			let mut fTemp89: F32 = fTemp88 + -1.0;
			let mut iTemp90: i32 = ((fTemp89 < 0.0) as i32);
			self.fRec39[0] = if (iTemp90 as i32 != 0) { fTemp88 } else { fTemp89 };
			let mut fThen13: F32 = fTemp87 + self.fRec39[1] + fTemp89 * (1.0 - self.fConst0 / fTemp86);
			let mut fRec40: F32 = if (iTemp90 as i32 != 0) { fTemp88 } else { fThen13 };
			let mut fTemp91: F32 = F32::max(1.1920929e-07, F32::abs(440.0 * fTemp43 * fTemp85));
			let mut fTemp92: F32 = self.fConst5 * fTemp91;
			let mut fTemp93: F32 = self.fRec42[1] + fTemp92;
			let mut fTemp94: F32 = fTemp93 + -1.0;
			let mut iTemp95: i32 = ((fTemp94 < 0.0) as i32);
			self.fRec42[0] = if (iTemp95 as i32 != 0) { fTemp93 } else { fTemp94 };
			let mut fThen15: F32 = self.fRec42[1] + fTemp92 + fTemp94 * (1.0 - self.fConst0 / fTemp91);
			let mut fRec43: F32 = if (iTemp95 as i32 != 0) { fTemp93 } else { fThen15 };
			let mut fTemp96: F32 = F32::max(1.1920929e-07, F32::abs(440.0 * fTemp48 * fTemp85));
			let mut fTemp97: F32 = self.fConst5 * fTemp96;
			let mut fTemp98: F32 = self.fRec44[1] + fTemp97;
			let mut fTemp99: F32 = fTemp98 + -1.0;
			let mut iTemp100: i32 = ((fTemp99 < 0.0) as i32);
			self.fRec44[0] = if (iTemp100 as i32 != 0) { fTemp98 } else { fTemp99 };
			let mut fThen17: F32 = fTemp97 + self.fRec44[1] + fTemp99 * (1.0 - self.fConst0 / fTemp96);
			let mut fRec45: F32 = if (iTemp100 as i32 != 0) { fTemp98 } else { fThen17 };
			let mut fTemp101: F32 = F32::tan(self.fConst8 * F32::max(20.0, F32::min(10000.0, 440.0 * F32::powf(2.0, 0.0833333358 * (self.fRec25[0] + self.fRec41[0] + -69.0)))));
			let mut fTemp102: F32 = 1.0 / fTemp101;
			let mut fTemp103: F32 = 1.0 - 1.0 / mydsp_faustpower2_f(fTemp101);
			let mut fTemp104: F32 = fTemp55 + (fTemp59 + fTemp102) / fTemp101 + 1.0;
			self.fRec38[0] = 2.0 * fRec40 + self.fRec18[0] * (0.0 - 2.0 * (1.0 - (fRec43 + fRec45))) + -1.0 - (self.fRec38[2] * (fTemp55 + 1.0 - (fTemp59 - fTemp102) / fTemp101) + 2.0 * self.fRec38[1] * (fTemp55 + fTemp103)) / fTemp104;
			let mut fTemp105: F32 = fTemp62 + (fTemp63 + fTemp102) / fTemp101 + 1.0;
			self.fRec37[0] = (self.fRec38[2] + self.fRec38[0] + 2.0 * self.fRec38[1]) / fTemp104 - (self.fRec37[2] * (fTemp62 + (fTemp102 - fTemp63) / fTemp101 + 1.0) + 2.0 * self.fRec37[1] * (fTemp62 + fTemp103)) / fTemp105;
			self.fRec46[0] = fSlow14 + self.fConst3 * self.fRec46[1];
			self.fRec51[0] = fSlow15 + self.fConst3 * self.fRec51[1];
			let mut fTemp106: F32 = F32::powf(2.0, 0.0833333358 * (fSlow4 + self.fRec51[0] + -69.0));
			let mut fTemp107: F32 = F32::max(1.1920929e-07, F32::abs(440.0 * fTemp106));
			let mut fTemp108: F32 = self.fConst5 * fTemp107;
			let mut fTemp109: F32 = self.fRec49[1] + fTemp108;
			let mut fTemp110: F32 = fTemp109 + -1.0;
			let mut iTemp111: i32 = ((fTemp110 < 0.0) as i32);
			self.fRec49[0] = if (iTemp111 as i32 != 0) { fTemp109 } else { fTemp110 };
			let mut fThen19: F32 = fTemp108 + self.fRec49[1] + fTemp110 * (1.0 - self.fConst0 / fTemp107);
			let mut fRec50: F32 = if (iTemp111 as i32 != 0) { fTemp109 } else { fThen19 };
			let mut fTemp112: F32 = F32::max(1.1920929e-07, F32::abs(440.0 * fTemp43 * fTemp106));
			let mut fTemp113: F32 = self.fRec52[1] + self.fConst5 * fTemp112;
			let mut fTemp114: F32 = fTemp113 + -1.0;
			let mut iTemp115: i32 = ((fTemp114 < 0.0) as i32);
			self.fRec52[0] = if (iTemp115 as i32 != 0) { fTemp113 } else { fTemp114 };
			let mut fThen21: F32 = fTemp113 + fTemp114 * (1.0 - self.fConst0 / fTemp112);
			let mut fRec53: F32 = if (iTemp115 as i32 != 0) { fTemp113 } else { fThen21 };
			let mut fTemp116: F32 = F32::max(1.1920929e-07, F32::abs(440.0 * fTemp48 * fTemp106));
			let mut fTemp117: F32 = self.fConst5 * fTemp116;
			let mut fTemp118: F32 = self.fRec54[1] + fTemp117;
			let mut fTemp119: F32 = fTemp118 + -1.0;
			let mut iTemp120: i32 = ((fTemp119 < 0.0) as i32);
			self.fRec54[0] = if (iTemp120 as i32 != 0) { fTemp118 } else { fTemp119 };
			let mut fThen23: F32 = self.fRec54[1] + fTemp117 + fTemp119 * (1.0 - self.fConst0 / fTemp116);
			let mut fRec55: F32 = if (iTemp120 as i32 != 0) { fTemp118 } else { fThen23 };
			let mut fTemp121: F32 = F32::tan(self.fConst8 * F32::max(20.0, F32::min(10000.0, 440.0 * F32::powf(2.0, 0.0833333358 * (self.fRec25[0] + self.fRec51[0] + -69.0)))));
			let mut fTemp122: F32 = 1.0 / fTemp121;
			let mut fTemp123: F32 = 1.0 - 1.0 / mydsp_faustpower2_f(fTemp121);
			let mut fTemp124: F32 = fTemp55 + (fTemp59 + fTemp122) / fTemp121 + 1.0;
			self.fRec48[0] = 2.0 * fRec50 + self.fRec18[0] * (0.0 - 2.0 * (1.0 - (fRec53 + fRec55))) + -1.0 - (self.fRec48[2] * (fTemp55 + 1.0 - (fTemp59 - fTemp122) / fTemp121) + 2.0 * self.fRec48[1] * (fTemp55 + fTemp123)) / fTemp124;
			let mut fTemp125: F32 = fTemp62 + (fTemp63 + fTemp122) / fTemp121 + 1.0;
			self.fRec47[0] = (self.fRec48[2] + self.fRec48[0] + 2.0 * self.fRec48[1]) / fTemp124 - (self.fRec47[2] * (fTemp62 + (fTemp122 - fTemp63) / fTemp121 + 1.0) + 2.0 * self.fRec47[1] * (fTemp62 + fTemp123)) / fTemp125;
			self.fRec0[(self.IOTA0 & 65535) as usize] = self.fConst1 * self.fRec1[0] * (self.fRec2[0] * fTemp1 + self.fRec5[0] * fTemp10 + self.fRec7[0] * fTemp19 + self.fRec9[0] * fTemp28) + 0.300000012 * self.fRec0[((self.IOTA0 - self.iConst7) & 65535) as usize] + 0.5 * self.fRec11[0] * ((self.fRec12[0] * (self.fRec13[2] + self.fRec13[0] + 2.0 * self.fRec13[1])) / fTemp64 + (self.fRec26[0] * (self.fRec27[2] + self.fRec27[0] + 2.0 * self.fRec27[1])) / fTemp84 + (self.fRec36[0] * (self.fRec37[2] + self.fRec37[0] + 2.0 * self.fRec37[1])) / fTemp105 + (self.fRec46[0] * (self.fRec47[2] + self.fRec47[0] + 2.0 * self.fRec47[1])) / fTemp125);
			self.fRec57[0] = fSlow17 + self.fConst3 * self.fRec57[1];
			let mut fTemp126: F32 = F32::powf(2.0, 0.0833333358 * (fSlow4 + self.fRec57[0] + -69.0));
			let mut fTemp127: F32 = F32::tan(self.fConst8 * F32::max(20.0, F32::min(10000.0, 1760.0 * fTemp126)));
			let mut fTemp128: F32 = 1.0 / fTemp127;
			let mut fTemp129: F32 = (fTemp128 + -2.0) / fTemp127 + 1.0;
			let mut fTemp130: F32 = 1.0 - 1.0 / mydsp_faustpower2_f(fTemp127);
			self.fRec61[0] = fSlow18 + self.fConst3 * self.fRec61[1];
			...
			self.fVec9[0] = fSlow19;
			self.iRec65[0] = (self.iRec65[1] + ((self.iRec65[1] > 0) as i32)) * ((fSlow19 <= self.fVec9[1]) as i32) + ((fSlow19 > self.fVec9[1]) as i32);
			let mut fTemp131: F32 = self.fConst9 * ((self.iRec65[0]) as F32);
			let mut fTemp132: F32 = 4.65661287e-10 * ((self.iRec64[0]) as F32) * F32::max(0.0, F32::min(fTemp131, 2.0 - fTemp131)) - self.fRec62[1] * fSlow22;
			self.fVec10[(self.IOTA0 & 2047) as usize] = fTemp132;
			let mut fTemp133: F32 = self.fConst10 / fTemp126;
			let mut fTemp134: F32 = fTemp133 + -1.0;
			let mut iTemp135: i32 = ((fTemp134) as i32);
			let mut fTemp136: F32 = F32::floor(fTemp134);
			self.fRec62[0] = self.fVec10[((self.IOTA0 - std::cmp::min(1025, std::cmp::max(0, iTemp135))) & 2047) as usize] * (fTemp136 + 2.0 - fTemp133) + (fTemp133 + -1.0 - fTemp136) * self.fVec10[((self.IOTA0 - std::cmp::min(1025, std::cmp::max(0, iTemp135 + 1))) & 2047) as usize];
			self.fRec63[0] = fTemp132;
			let mut fTemp137: F32 = F32::powf(2.0, 2.29999995 * self.fRec61[0]);
			let mut fTemp138: F32 = 1.0 - self.fConst11 * fTemp137 / F32::powf(2.0, 2.0 * (1.0 - self.fRec61[0]) + 1.0);
			self.fRec66[0] = 0.999000013 * self.fRec66[1] - 0.00200000009 * fTemp138 * F32::cos(self.fConst12 * fTemp137);
			self.fRec67[0] = 0.999000013 * self.fRec67[1] + 0.00100000005 * mydsp_faustpower2_f(fTemp138);
			self.fRec59[0] = self.fRec60[0] * self.fRec63[1] - (self.fRec66[0] * self.fRec59[1] + self.fRec67[0] * self.fRec59[2]);
			let mut fTemp139: F32 = (fTemp128 + 2.0) / fTemp127 + 1.0;
			self.fRec58[0] = self.fRec59[0] - (self.fRec59[1] + (self.fRec58[2] * fTemp129 + 2.0 * self.fRec58[1] * fTemp130) / fTemp139);
			self.fRec56[0] = 0.0 - ((fTemp129 * self.fRec56[2] + 2.0 * fTemp130 * self.fRec56[1]) - (self.fRec58[2] + self.fRec58[0] + 2.0 * self.fRec58[1])) / fTemp139;
			let mut fTemp140: F32 = self.fRec0[(self.IOTA0 & 65535) as usize] + fSlow16 * (self.fRec56[2] + self.fRec56[0] + 2.0 * self.fRec56[1]) / fTemp139;
			*output0 = ((fTemp140) as F32);
			*output1 = ((fTemp140) as F32);
