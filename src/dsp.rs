...
impl FaustDsp for mydsp {
	...
	fn compute(&mut self, count: i32, inputs: &[&[Self::T]], outputs: &mut[&mut[Self::T]]) {
		...
		for (output0, output1) in zipped_iterators {
			...
			let mut fTemp44: F32 = self.fRec18[1] + fTemp43;
			...
			let mut fThen3: F32 = self.fRec18[1] + fTemp43 + fTemp45 * (1.0 - self.fConst0 / fTemp42);
			...
			let mut fTemp48: F32 = self.fConst5 * fTemp47;
			let mut fTemp49: F32 = self.fRec21[1] + fTemp48;
			let mut fTemp50: F32 = fTemp49 + -1.0;
			let mut iTemp51: i32 = ((fTemp50 < 0.0) as i32);
			self.fRec21[0] = if (iTemp51 as i32 != 0) { fTemp49 } else { fTemp50 };
			let mut fThen5: F32 = fTemp48 + self.fRec21[1] + fTemp50 * (1.0 - self.fConst0 / fTemp47);
			let mut fRec22: F32 = if (iTemp51 as i32 != 0) { fTemp49 } else { fThen5 };
			self.fRec23[0] = fSlow6 + self.fConst3 * self.fRec23[1];
			let mut fTemp52: F32 = F32::powf(2.0, 0.0833333358 * (self.fRec16[0] + -81.0));
			let mut fTemp53: F32 = F32::max(440.0 * fTemp52, 23.4489498);
			...
			let mut fTemp72: F32 = 0.772727251 / fTemp52 + -0.100000001;
			...
			let mut fTemp125: F32 = F32::tan(self.fConst11 * fTemp52);
			...
			let mut fTemp128: F32 = ((self.iRec59[0]) as F32) / F32::max(1.0, self.fConst12 * mydsp_faustpower2_f(1.0 - 0.146666661 * fTemp52));
