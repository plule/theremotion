...
impl FaustDsp for mydsp {
	...
	fn compute(&mut self, count: i32, inputs: &[&[Self::T]], outputs: &mut[&mut[Self::T]]) {
		...
		for (output0, output1) in zipped_iterators {
			...
			let mut fTemp49: F32 = self.fRec21[1] + self.fConst5 * fTemp48;
			...
			let mut fThen5: F32 = fTemp49 + fTemp50 * (1.0 - self.fConst0 / fTemp48);
			let mut fRec22: F32 = if (iTemp51 as i32 != 0) { fTemp49 } else { fThen5 };
			self.fRec23[0] = fSlow6 + self.fConst3 * self.fRec23[1];
			let mut fTemp52: F32 = F32::max(440.0 * F32::powf(2.0, 0.0833333358 * (self.fRec16[0] + -81.0)), 23.4489498);
			let mut fTemp53: F32 = F32::max(20.0, F32::abs(fTemp52));
			let mut fTemp54: F32 = self.fRec24[1] + self.fConst5 * fTemp53;
			self.fRec24[0] = fTemp54 - F32::floor(fTemp54);
			let mut fTemp55: F32 = mydsp_faustpower2_f(2.0 * self.fRec24[0] + -1.0);
			self.fVec9[0] = fTemp55;
			let mut fTemp56: F32 = (fTemp0 * (fTemp55 - self.fVec9[1])) / fTemp53;
			self.fVec10[(self.IOTA0 & 4095) as usize] = fTemp56;
			let mut fTemp57: F32 = F32::max(0.0, F32::min(2047.0, self.fConst6 / fTemp52));
			...
			let mut fTemp62: F32 = 1.41421354 * fTemp60;
			...
			let mut fTemp66: F32 = 1.0 - 1.0 / mydsp_faustpower2_f(fTemp64);
			let mut fTemp67: F32 = fTemp61 + (fTemp65 + fTemp63) / fTemp64 + 1.0;
			self.fRec13[0] = 2.0 * fRec15 + self.fRec17[0] * (0.0 - 2.0 * (1.0 - (fRec19 + fRec22))) + self.fConst4 * self.fRec23[0] * (fTemp56 - self.fVec10[((self.IOTA0 - iTemp58) & 4095) as usize] * (fTemp59 + 1.0 - fTemp57) - (fTemp57 - fTemp59) * self.fVec10[((self.IOTA0 - (iTemp58 + 1)) & 4095) as usize]) + -1.0 - (self.fRec13[2] * (fTemp61 + 1.0 - (fTemp63 - fTemp65) / fTemp64) + 2.0 * self.fRec13[1] * (fTemp61 + fTemp66)) / fTemp67;
			let mut fTemp68: F32 = fTemp60 * (fTemp60 + -1.41421354);
			let mut fTemp69: F32 = 2.0 - fTemp62;
			let mut fTemp70: F32 = fTemp68 + (fTemp69 + fTemp65) / fTemp64 + 1.0;
			self.fRec12[0] = (self.fRec13[2] + self.fRec13[0] + 2.0 * self.fRec13[1]) / fTemp67 - (self.fRec12[2] * (fTemp68 + (fTemp65 - fTemp69) / fTemp64 + 1.0) + 2.0 * self.fRec12[1] * (fTemp68 + fTemp66)) / fTemp70;
			self.fRec0[(self.IOTA0 & 65535) as usize] = self.fConst1 * self.fRec1[0] * (self.fRec2[0] * fTemp1 + self.fRec5[0] * fTemp10 + self.fRec7[0] * fTemp19 + self.fRec9[0] * fTemp28) + 0.300000012 * self.fRec0[((self.IOTA0 - self.iConst7) & 65535) as usize] + (self.fRec11[0] * (self.fRec12[2] + self.fRec12[0] + 2.0 * self.fRec12[1])) / fTemp70;
			self.iRec37[0] = 0;
			...
			let mut fTemp71: F32 = 0.772727251 / fTemp37 + -0.100000001;
			...
			let mut fTemp124: F32 = F32::tan(self.fConst11 * fTemp37);
			...
			let mut fTemp127: F32 = ((self.iRec59[0]) as F32) / F32::max(1.0, self.fConst12 * mydsp_faustpower2_f(1.0 - 0.146666661 * fTemp37));
